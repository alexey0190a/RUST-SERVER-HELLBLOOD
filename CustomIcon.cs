 ...
 private void TryApplySteamAvatarUserID(string command, object[] args)
{
    ...
    // Avatar: подменяем только если userId == 0
    ulong providedID;
    if (ulong.TryParse(args[1]?.ToString(), out providedID) && providedID == 0)
        args[1] = _configuration.SteamAvatarUserID;

    // Name: ищем SERVER среди аргументов и меняем на конфиг
    for (int i = 0; i < args.Length; i++)
    {
        var s = args[i]?.ToString();
        if (string.IsNullOrEmpty(s)) continue;

        if (s.Equals("SERVER", System.StringComparison.OrdinalIgnoreCase))
        {
            args[i] = string.IsNullOrEmpty(_configuration.Name) ? "SERVER" : _configuration.Name;
            break;
        }
    }
}
